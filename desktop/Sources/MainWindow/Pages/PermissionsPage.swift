        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 24) {
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    Text("Marrow needs the following permissions to work properly.")
                        .scaledFont(size: 14)
                ...
                VStack(spacing: 20) {
                    ...
                    // Accessibility Permission (required for execution actions)
                ...
                if !appState.hasMissingPermissions {
                    HStack(spacing: 12) {
                        ...
                        Text("All permissions granted! Marrow is ready to use.")
        ...
        VStack(alignment: .leading, spacing: 16) {
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                HStack(alignment: .top, spacing: 8) {
                    ...
                    VStack(alignment: .leading, spacing: 6) {
                        Text("Find \"Marrow\" and toggle it ON")
                            .scaledFont(size: 13)
                            .foregroundColor(OmiColors.textSecondary)
...
struct AccessibilityPermissionSection: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            HStack(spacing: 16) {
                ZStack {
                    Circle()
                        .fill(appState.hasAccessibilityPermission ? Color.green.opacity(0.15) : OmiColors.backgroundTertiary)
                        .frame(width: 48, height: 48)
                    Image(systemName: "hand.tap.fill")
                        .scaledFont(size: 22)
                        .foregroundColor(appState.hasAccessibilityPermission ? .green : OmiColors.textSecondary)
                }
                ...
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    Text("Required for Marrow to control apps, type, click, and run automations on your behalf.")
                        .scaledFont(size: 13)
                        .foregroundColor(OmiColors.textTertiary)
                }
                Spacer()
            ...
            if !appState.hasAccessibilityPermission || appState.isAccessibilityBroken {
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    VStack(alignment: .leading, spacing: 10) {
                        instructionStep(number: 1, text: "Open System Settings > Privacy & Security > Accessibility")
                        instructionStep(number: 2, text: "Find \"Marrow\" and toggle it ON")
                        instructionStep(number: 3, text: "Restart Marrow for the change to take effect")
                    }
                    ...
                    Button(action: openAccessibilitySettings) {
                        ...
                        .padding(.vertical, 10)
                        .frame(maxWidth: .infinity)
                        .background(RoundedRectangle(cornerRadius: 10).fill(OmiColors.purplePrimary))
                    }
                    .buttonStyle(.plain)
                }
                .padding(.horizontal, 20)
        ...
        .background(
            RoundedRectangle(cornerRadius: 16)
                ...
                .overlay(
                    RoundedRectangle(cornerRadius: 16)
                        .stroke(appState.hasAccessibilityPermission ? Color.green.opacity(0.3) : OmiColors.backgroundQuaternary.opacity(0.5), lineWidth: 1)
                )
        )
    }
    ...
    private var accessibilityStatusBadge: some View {
        HStack(spacing: 4) {
            Image(systemName: appState.hasAccessibilityPermission ? "checkmark.circle.fill" : "exclamationmark.circle.fill")
                .scaledFont(size: 12)
            Text(appState.hasAccessibilityPermission ? (appState.isAccessibilityBroken ? "Broken" : "Granted") : "Not Granted")
                .scaledFont(size: 12, weight: .medium)
        }
        .foregroundColor(appState.hasAccessibilityPermission && !appState.isAccessibilityBroken ? .green : OmiColors.warning)
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(
            Capsule()
                .fill((appState.hasAccessibilityPermission && !appState.isAccessibilityBroken) ? Color.green.opacity(0.15) : OmiColors.warning.opacity(0.15))
        )
    }
