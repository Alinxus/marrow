...
class ChatLabViewModel: ObservableObject {
    ...
    private func fetchRatingsFromBackend() async -> [(String, Int, Int)] {
        do {
            ...
            let url = URL(string: "http://localhost:8888/v2/messages?limit=500")!
    ...
    private func callClaude(systemPrompt: String, userMessage: String) async -> (String, Int, String) {
        do {
            let url = URL(string: "http://localhost:8888/v1/chat/messages")!
            var request = URLRequest(url: url)
            ...
            let body: [String: Any] = [
                "text": userMessage,
                "system_prompt": String(systemPrompt.prefix(50000)),
    ...
    private func gradeResponse(question: String, response: String) async -> (Int, String) {
        ...
        do {
            ...
            let body: [String: Any] = ["text": gradePrompt, "model": "claude-haiku-4-5"]
            request.httpBody = try JSONSerialization.data(withJSONObject: body)
            let (data, _) = try await URLSession.shared.data(for: request)
            let json = try JSONSerialization.jsonObject(with: data) as? [String: Any]
            ...
            {
                return (grade["score"] as? Int ?? 0, grade["comment"] as? String ?? "")
            }
