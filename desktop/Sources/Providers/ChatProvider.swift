...
"""
    ...
    private lazy var acpBridge: ACPBridge = {
        let isOmi = (UserDefaults.standard.string(forKey: "chatBridgeMode") ?? BridgeMode.marrowLocal.rawValue) != BridgeMode.userClaude.rawValue
        return ACPBridge(passApiKey: isOmi)
    }()
    private var acpBridgeStarted = false
    private var marrowBridge = MarrowLocalBridge()

    enum BridgeMode: String {
        case omiAI = "agentSDK"
        case userClaude = "claudeCode"
        case marrowLocal = "marrowLocal"
    }
    @AppStorage("chatBridgeMode") var bridgeMode: String = BridgeMode.marrowLocal.rawValue
    ...
    private func ensureBridgeStarted() async -> Bool {
        // marrowLocal talks directly to the Python server — no Node.js bridge needed
        if bridgeMode == BridgeMode.marrowLocal.rawValue { return true }
    ...
    /// Switch between bridge modes (Omi AI vs user's Claude account vs local Marrow server)
    func switchBridgeMode(to mode: BridgeMode) async {
        // marrowLocal uses no ACP bridge — just update the stored mode and return
        if mode == .marrowLocal {
    ...
    func disconnectClaude() async {
        ...
        bridgeMode = BridgeMode.marrowLocal.rawValue
    }
    ...
    func labRunQuestion(question: String, systemPrompt: String, sessionKey: String) async -> String {
        ...
        do {
            ...
            let result: ACPBridge.QueryResult
            if bridgeMode == BridgeMode.marrowLocal.rawValue {
                result = try await marrowBridge.query(
                    prompt: question,
                ...
                result = try await acpBridge.query(
                    prompt: question,
                    systemPrompt: systemPrompt,
                    sessionKey: sessionKey,
    ...
    func sendMessage(_ text: String, model: String? = nil, isFollowUp: Bool = false, systemPromptSuffix: String? = nil, systemPromptPrefix: String? = nil, sessionKey: String? = nil, resume: String? = nil, imageData: Data? = nil) async {
        ...
        do {
            ...
            let effectiveSessionKey = isOnboarding ? "onboarding" : (sessionKey ?? "main")
            let queryResult: ACPBridge.QueryResult
            if bridgeMode == BridgeMode.marrowLocal.rawValue {
                queryResult = try await marrowBridge.query(
                    prompt: trimmedText,
                    systemPrompt: systemPrompt,
